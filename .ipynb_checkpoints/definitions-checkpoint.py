...
# Below is old code that does not seem to work
"""
def Riemann_all_lower(metric_tensor, inverse_metric_tensor, coordinates):
    num_coordinates = len(coordinates)
    riemann_full = Riemann(metric_tensor, inverse_metric_tensor, coordinates)

    riemann_lower = [[[[
        sum(
            riemann_full[i][m][j][k] * inverse_metric_tensor[m, l]
            for m in range(num_coordinates)
        )
        for l in range(num_coordinates)
    ] for k in range(num_coordinates)] for j in range(num_coordinates)] for i in range(num_coordinates)]

    return riemann_lower
"""

def Riemann_all_lower(metric_tensor, inverse_metric_tensor, riemann_upper, coordinates):
    num_coordinates = len(coordinates)
    
    # Compute Riemann curvature tensor with four lower indices
    riemann_lower = [[[[
        sum(
            metric_tensor[i, m] * riemann_upper[m][j][k][l]
            for m in range(num_coordinates)
        )
        for l in range(num_coordinates)
    ] for k in range(num_coordinates)] for j in range(num_coordinates)] for i in range(num_coordinates)]
